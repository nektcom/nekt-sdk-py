...
def get_cloud_provider(data_access_token: str, api_url: str = "https://api.nekt.ai") -> dict:
    """
    Get the cloud provider information from the Nekt API.
    """
    resp = requests.get(
        f"{api_url}/api/v1/organization/notebooks-cloud-provider/",
        headers={"X-Jupyter-Token": data_access_token},
    )
    return resp.json()
